...
import datetime
import tempfile # Generally useful, though not strictly needed for Option 2 directly

# Import and load dotenv for local development with .env file
...
# Ensure forwarding_works.py is in the same directory and contains 'initialize_firebase_app'.
...
# --- Google Cloud Credentials Handling (using .env paths to JSON files) ---
GOOGLE_CLOUD_KEY_PATH: Optional[str] = None

# Get Google Cloud Service Account JSON file path from environment variables
# This will read "key.json" from your .env
gcp_sa_key_file_path: Optional[str] = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS")

if not gcp_sa_key_file_path:
    st.error("Environment variable 'GOOGLE_APPLICATION_CREDENTIALS' not found. Please add the path to your Google Service Account JSON key (e.g., './key.json') to your .env file.")
    st.stop()

# Resolve to absolute path for robustness
absolute_gcp_path = os.path.abspath(gcp_sa_key_file_path)

# Verify if the file exists
if not os.path.exists(absolute_gcp_path):
    st.error(f"Google Cloud credentials file not found at path: {absolute_gcp_path}. Please check your .env and file location.")
    st.stop()

# Initialize gcp_file_content to an empty string to satisfy static analysis
gcp_file_content: str = ""

# --- DEBUGGING FOR GOOGLE CLOUD JSON ---
print("\n--- Google Cloud Credential Debug ---")
print(f"DEBUG: Path from .env (GOOGLE_APPLICATION_CREDENTIALS): {gcp_sa_key_file_path}")
print(f"DEBUG: Absolute path: {absolute_gcp_path}")
try:
    with open(absolute_gcp_path, 'r', encoding='utf-8') as f:
        gcp_file_content = f.read() # This is where it gets assigned
        print(f"DEBUG: Google Cloud file content length: {len(gcp_file_content)} chars")
        print(f"DEBUG: Google Cloud file starts with: '{gcp_file_content[:50]}...'")
        # Attempt to parse it here to catch JSON errors early
        parsed_gcp_json = json.loads(gcp_file_content)
        print("DEBUG: Google Cloud file content is valid JSON.")
        print(f"DEBUG: Google Cloud Project ID from JSON: {parsed_gcp_json.get('project_id', 'Not Found')}")
except FileNotFoundError:
    # This block might be redundant due to the os.path.exists check above, but good for robustness
    print(f"DEBUG: Google Cloud file not found at {absolute_gcp_path} (despite initial os.path.exists check).")
    st.error("Internal error: Google Cloud credentials file disappeared or became inaccessible.")
    st.stop()
except json.JSONDecodeError as e:
    print(f"DEBUG: JSON DECODE ERROR in Google Cloud file {absolute_gcp_path}: {e}")
    print(f"Full Google Cloud content (up to 500 chars):\n{gcp_file_content[:500]}...")
    st.error(f"Error: Google Cloud credentials JSON file is invalid. Details: {e}. Check '{gcp_sa_key_file_path}' carefully.")
    st.stop()
except Exception as e:
    print(f"DEBUG: An unexpected error occurred reading Google Cloud file: {e}")
    st.error(f"An unexpected error occurred with Google Cloud credentials: {e}")
    st.stop()
# --- END DEBUGGING FOR GOOGLE CLOUD JSON ---

try:
    # Set the GOOGLE_APPLICATION_CREDENTIALS environment variable directly to the path
    # Google Cloud libraries will automatically pick this up.
    os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = absolute_gcp_path
    GOOGLE_CLOUD_KEY_PATH = absolute_gcp_path # Assign the path for your get_translator_client etc.
except Exception as e:
    st.error(f"Error setting Google Cloud credentials path from environment variables: {e}")
    st.stop()

# Get Google Cloud Project ID from environment variables
_GC_PROJECT_ID: Optional[str] = os.environ.get("GOOGLE_CLOUD_PROJECT")
if not _GC_PROJECT_ID:
    st.error("Environment variable 'GOOGLE_CLOUD_PROJECT' not found. Please add it to your .env file.")
    st.stop()
...
# --- Firebase Service Account Key Handling (using .env paths to JSON files) ---
FIREBASE_SERVICE_ACCOUNT_KEY_PATH: Optional[str] = None
# This will read "animal-bites.json" from your .env
firebase_sa_key_file_path: Optional[str] = os.environ.get("FIREBASE_SERVICE_ACCOUNT_KEY")

# Flag to track if Firebase should be initialized
should_initialize_firebase = True

if not firebase_sa_key_file_path:
    st.warning("Environment variable 'FIREBASE_SERVICE_ACCOUNT_KEY' not found. Firebase features may not be initialized.")
    should_initialize_firebase = False
else:
    absolute_firebase_path = os.path.abspath(firebase_sa_key_file_path)
    # Verify if the Firebase file exists
    if not os.path.exists(absolute_firebase_path):
        st.warning(f"Firebase service account file not found at path: {absolute_firebase_path}. Firebase features will be disabled.")
        should_initialize_firebase = False
    else:
        # Initialize firebase_file_content to an empty string to satisfy static analysis
        firebase_file_content: str = ""
        # --- DEBUGGING FOR FIREBASE JSON ---
        print("\n--- Firebase Credential Debug ---")
        print(f"DEBUG: Path from .env (FIREBASE_SERVICE_ACCOUNT_KEY): {firebase_sa_key_file_path}")
        print(f"DEBUG: Absolute path: {absolute_firebase_path}")
        try:
            with open(absolute_firebase_path, 'r', encoding='utf-8') as f:
                firebase_file_content = f.read()
                print(f"DEBUG: Firebase file content length: {len(firebase_file_content)} chars")
                print(f"DEBUG: Firebase file starts with: '{firebase_file_content[:50]}...'")
                # Attempt to parse it here to catch JSON errors early
                parsed_firebase_json = json.loads(firebase_file_content)
                print("DEBUG: Firebase file content is valid JSON.")
                print(f"DEBUG: Firebase Project ID from JSON: {parsed_firebase_json.get('project_id', 'Not Found')}")
            # If we reach here, file exists and is valid JSON, so set the path
            FIREBASE_SERVICE_ACCOUNT_KEY_PATH = absolute_firebase_path

        except FileNotFoundError:
            # This block might be redundant due to the os.path.exists check above
            print(f"DEBUG: Firebase file not found at {absolute_firebase_path} (despite initial os.path.exists check).")
            st.warning("Internal error: Firebase credentials file disappeared or became inaccessible. Firebase features will be disabled.")
            should_initialize_firebase = False
        except json.JSONDecodeError as e:
            print(f"DEBUG: JSON DECODE ERROR in Firebase file {absolute_firebase_path}: {e}")
            print(f"Full Firebase content (up to 500 chars):\n{firebase_file_content[:500]}...")
            st.error(f"Error reading Firebase credentials JSON file: {e}. Firebase features will be disabled. Check '{firebase_sa_key_file_path}' carefully.")
            should_initialize_firebase = False
        except Exception as e:
            print(f"DEBUG: An unexpected error occurred reading Firebase file: {e}")
            st.error(f"An unexpected error occurred with Firebase credentials: {e}. Firebase features will be disabled.")
            should_initialize_firebase = False
...
openai_api_key: Optional[str] = os.environ.get("OPENAI_KEY")
if not openai_api_key:
    st.error("Environment variable 'OPENAI_KEY' not found. Please add it to your .env file.")
    st.stop()
...
# --- MongoDB Initialization (using .env variables) ---
mongodb_uri: Optional[str] = os.environ.get("MONGODB_URI")
if not mongodb_uri:
    st.error("Environment variable 'MONGODB_URI' not found. Please add it to your .env file.")
    st.stop()
...
try:
    ...
    st.error(f"Failed to connect to MongoDB: {e}. Please check your MONGODB_URI environment variable and ensure MongoDB is accessible.")
    st.stop()
