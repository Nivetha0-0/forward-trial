...
def get_firebase_config():
    ...
    project_id = os.getenv('FIREBASE_SERVICE_ACCOUNT_KEY')
    if not project_id:
        raise ValueError(
...
def initialize_firebase():
    if firebase_admin._apps:
        print("ℹ  Firebase already initialized, using existing app")
...
def save_unanswered_question(question_english):
    try:
        ...
        if question_english not in qn_list:
            ...
            print(f"ℹ  Question already exists in unanswered list: {question_english}")
