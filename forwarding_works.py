...
firebase_sa_key = st.secrets["FIREBASE_SERVICE_ACCOUNT_KEY"]

# Initialize Firebase if not already initialized
if not firebase_admin._apps:
    # Handle both string and dict formats for the service account key
    if isinstance(firebase_sa_key, str):
        firebase_sa_key = json.loads(firebase_sa_key)
    
    # Create temporary file with credentials
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".json") as temp_file:
        json.dump(firebase_sa_key, temp_file)
        temp_file.flush()
